...
# 版本：v2.1 - 修正版（修复跟随系数口径：amount→volume，跟随窗口不包含优势时刻本身）
#
# 【v2.1修正内容】
#   1. 跟随系数计算口径修正：amount（成交额）→ volume（成交量），与研报原文"成交量"对齐
#   2. 跟随窗口修正：从"包含优势时刻本身"改为"仅包含优势时刻之后的5分钟"，与研报原文"之后"对齐
#
# 因子构建逻辑（严格按研报原文）：
...
def filter_advantage_moments(df: pd.DataFrame, 
    ...
    for i in range(1, len(massive_indices)):
        ...
        last_valid_time = massive_df.loc[valid_indices[-1], 'date']
        
        # 计算时间差（分钟）
        time_diff = (current_time - last_valid_time).total_seconds() / 60
    ...
    if df_daily is None or df_daily.empty:
        ...
        if df_factor is None or df_factor.empty:
            ...
            if not df_factor.empty:
                ...
                print(f"\nA10待著而救因子 v2.1 - 严格按研报原文修正（跟随系数: volume口径，跟随窗口: 不含优势时刻本身）")
