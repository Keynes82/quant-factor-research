...
# 版本：v2.0_旧合成 - 基于高性能模板（多进程并行优化）
# ⚠️ 本版本为旧合成逻辑备份，合成方式为：先分别做 mean+std 再合成
#
# 因子构建逻辑：
...
#      【旧合成】月度波动率 = (mean + std) / 2，月度收益率同理，再取均值
#
# 优化点：
#   - 多进程并行计算（8核）
#   - 批量SQL查询（减少IO）
...
def calculate_moderate_risk_factor_optimized(df_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    【v2.0_旧合成】合成方式：
    月度波动率 = (mean(moderate_vol,20日) + std(moderate_vol,20日)) / 2
    月度收益率 = (mean(moderate_ret,20日) + std(moderate_ret,20日)) / 2
    因子 = (月度波动率 + 月度收益率) / 2
    """
    ...
    # 【旧合成】向量化滚动计算：分别对波动率和收益率做 mean+std
    min_periods = min(10, FACTOR_WINDOW)  # 至少10天数据即可计算
    print(f"  滚动窗口: {FACTOR_WINDOW}日, 最小有效天数: {min_periods}日")
    
    df['monthly_moderate_vol_mean'] = df.groupby('instrument')['moderate_vol'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
    df['monthly_moderate_vol_std'] = df.groupby('instrument')['moderate_vol'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).std()
    )
    df['monthly_moderate_ret_mean'] = df.groupby('instrument')['moderate_ret'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
    df['monthly_moderate_ret_std'] = df.groupby('instrument')['moderate_ret'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).std()
    )
    
    # 【旧合成】合成因子（向量化）
    df['monthly_dazzling_vol'] = (df['monthly_moderate_vol_mean'] + df['monthly_moderate_vol_std']) / 2
    df['monthly_dazzling_ret'] = (df['monthly_moderate_ret_mean'] + df['monthly_moderate_ret_std']) / 2
    df['moderate_risk_factor'] = (df['monthly_dazzling_vol'] + df['monthly_dazzling_ret']) / 2
    
    # 只保留有效因子值
    df_result = df[df['moderate_risk_factor'].notna()].copy()
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（适度冒险）因子计算 - v2.0_旧合成 ===")
