...
# 因子名称：A06（飞蛾扑火）
# 版本：v1.1 - 修正截面均值计算BUG（2026-04-10）
#
# 修正说明：
...
def calculate_moth_to_flame_factor(daily_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for inst, group in df.groupby('instrument'):
        ...
        # 【修正】使用截面均值进行分类（而非历史均值）
        # 日跳跃度 < 截面均值：太阳型振幅（振幅 × -1）
        # 日跳跃度 > 截面均值：火把型振幅（振幅 × 1）
...
if __name__ == "__main__":
    ...
    print("2) 根据日跳跃度截面均值将振幅分类为'太阳型'和'火把型'")
    print("3) 太阳型振幅（无跳跃）翻转，火把型振幅（有跳跃）不变")
    print("4) 飞蛾扑火因子 = (月跳跃度 + 修正振幅) / 2")
