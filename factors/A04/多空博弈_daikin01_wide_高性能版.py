...
def calculate_minute_returns_vectorized(df: pd.DataFrame, window: int = RET_WINDOW) -> pd.DataFrame:
    """
    向量化计算过去N分钟收益率
    ...
    # 计算过去N分钟累计收益率（向量化rolling）
    df['past_return'] = df.groupby(['instrument', 'trade_date'])['minute_return'].transform(
...
def process_single_stock_optimized(inst: str, df_min: pd.DataFrame, 
    ...
    try:
        ...
        for trade_date, day_df in df_min.groupby('trade_date'):
            ...
            # 计算过去ret_window分钟累计收益率
            day_df['past_return'] = day_df['minute_return'].rolling(
                window=ret_window, min_periods=1
            ).sum()
...
def mean_distance_normalize(df: pd.DataFrame, factor_col: str) -> pd.DataFrame:
    ...
    # 截面均值距离化
    zscore_means = df.groupby('date')[factor_col + '_zscore'].transform('mean')
    df[factor_col + '_norm'] = (df[factor_col + '_zscore'] - zscore_means).abs()
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（多空博弈）因子计算 - 高性能优化版 ===")
