...
def fetch_stock_minute_data_batch(instruments: List[str], start_date: str, end_date: str) -> pd.DataFrame:
    ...
    WHERE date >= TIMESTAMP '{start_date} 09:30:00'
      ...
      AND (CAST(date AS TIME) >= '09:31:00' AND CAST(date AS TIME) <= '14:56:00')
...
def calculate_daily_night_frost(df_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    for current_date in all_dates:
        ...
        night_frost_values = np.mean(np.abs(corr_matrix), axis=1)
...
def calculate_daily_huayinlinjian(df_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_nf is None or df_nf.empty:
        ...
        # 只用两个子因子合成
        df['huayinlinjian'] = (df['morning_fog_20'].fillna(0) + df['noon_wood_20'].fillna(0)) / 2.0
    else:
        # merge夜眠霜路日频值
        ...
        # 等权合成（三个子因子rolling后）
        df['huayinlinjian'] = (
            df['morning_fog_20'].fillna(0) + 
            df['noon_wood_20'].fillna(0) + 
            df['night_frost_20'].fillna(0)
        ) / 3.0
