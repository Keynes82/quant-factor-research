...
#
# 因子逻辑：推动个股价格变化的因素分解
# 1. 将日内价格变化分解为成交量驱动部分和非成交量驱动部分
# 2. 计算非成交量驱动价格变化占比
# 3. 非成交量驱动占比越高，说明价格变化更多由信息驱动而非交易驱动
# 4. 日因子 = 非成交量驱动价格变化占比
# 5. 月因子 = 过去20日平均值
...
def calculate_price_volume_decomposition(df_day):
    """
    计算价格变化的因素分解
    
    将日内价格变化分解为：
    1. 成交量驱动部分（与成交量相关的变化）
    2. 非成交量驱动部分（信息驱动、与成交量无关的变化）
    ...
    df_day['volume_norm'] = (df_day['volume'] - df_day['volume'].mean()) / df_day['volume'].std()
    
    # 去除NaN
    df_clean = df_day.dropna(subset=['price_change', 'volume_norm'])
    
    if len(df_clean) < 10:
        return None
    
    price_changes = df_clean['price_change'].values
    volumes = df_clean['volume_norm'].values
    ...
    try:
        ...
        volume_driven_contrib = np.sum(np.abs(volume_driven))
        non_volume_driven_contrib = np.sum(np.abs(non_volume_driven))
        
        if total_price_change == 0:
            return None
        
        # 非成交量驱动占比（花隐林间因子核心）
        non_volume_ratio = non_volume_driven_contrib / total_price_change
        
        # R²（决定系数，表示成交量解释价格变化的程度）
        r_squared = r_value ** 2
        
        # 另一种计算：残差占比
        residual_ratio = np.sum(non_volume_driven ** 2) / np.sum(price_changes ** 2)
        
        return {
            'non_volume_ratio': non_volume_ratio,
            'volume_driven_ratio': 1 - non_volume_ratio,
            'r_squared': r_squared,
...
def process_single_stock(inst, df_min):
    ...
    try:
        ...
        for date, group in df_min.groupby(df_min['date'].dt.date):
            ...
            # 步骤1：计算价格-成交量分解
            decomposition = calculate_price_volume_decomposition(df_day)
            
            if decomposition is None:
                continue
            
            # 步骤2：提取花隐林间因子
            # 非成交量驱动占比越高，说明价格变化更多由信息驱动
            factor_value = decomposition['non_volume_ratio']
    ...
    if df_factor is not None and not df_factor.empty:
        ...
        if not df_factor.empty:
            ...
            if not df_factor.empty:
                ...
                print(f"'{FACTOR_NAME}' 花隐林间因子共写入 {written_count} 条数据到 '{FACTOR_LIBRARY_TABLE}'")
                print(f"\n预期表现:")
                print(f"  - Rank IC: -6.92%")
                print(f"  - Rank ICIR: -3.85")
                print(f"  - 多空年化收益: 28.76%")
                print(f"  - 信息比率: 3.15")
            else:
                print("过滤后没有数据需要写入。")
        else:
