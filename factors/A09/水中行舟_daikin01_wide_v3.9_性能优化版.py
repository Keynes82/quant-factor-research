# A09 水中行舟因子 - v3.9 性能优化版（daikin01）
# 来源：方正证券《个股成交额的市场跟随性与"水中行舟"因子》（2023-02-15）
# 档案编号：FQ-20260403-009
# 因子名称：A09（水中行舟）
# 版本：v3.9 - 性能优化版（窗口级Numba并行）
#
# 【v3.9 性能优化】
# 1. 【优化一】随波逐流窗口级Numba并行：
#    原Python循环每20日窗口调用一次Numba函数，编译+调用开销 × 窗口数(100~500)。
#    改为一次性传入全矩阵，Numba内部 prange 并行所有窗口，单次编译+调用。
#    预估提速：窗口级循环消除，Numba调用开销从 O(n_windows) → O(1)。
#
# 【v3.8 核心BUG修复保留】
# 1. 【修复一】合成公式方向纠正：随波逐流(+向) - 孤雁出群(-向) → 改为 孤雁出群 - 随波逐流
# 2. 【修复二】随波逐流Spearman：共同交易日全列非NaN → 逐对(pairwise)共同有效数据
# 3. 【修复三】孤雁出群Pearson：列均值填充NaN + 矩阵运算 → 逐对(pairwise)共同有效数据
# 4. 【修复四】删除截面均值填充，保持自然缺失
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（水中行舟）因子计算 - v3.9 性能优化版 ===")
    print(f"目标表: {FACTOR_LIBRARY_TABLE}（宽表格式）")
    print(f"计算范围: {start_date} ~ {end_date}")
    print(f"因子窗口: {FACTOR_WINDOW}日")
    print()
    print("【v3.9 性能优化】")
    print("1. 【优化一】随波逐流窗口级Numba并行：")
    print("   原Python循环每20日窗口调用一次Numba函数（编译+调用开销×窗口数）。")
    print("   改为一次性传入全矩阵，Numba内部 prange 并行所有窗口，单次编译+调用。")
    print()
