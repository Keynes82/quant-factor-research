# A09 水中行舟因子 - v3.8 BUG修复版（daikin01）
...
#
# 【v3.8 核心BUG修复】
# 1. 【修复一】合成公式方向纠正：随波逐流(+向) - 孤雁出群(-向) → 改为 孤雁出群 - 随波逐流
#    原公式导致IC符号翻转（+1.73%），与研报基准-9.36%方向相反。
#    随波逐流IC≈+5.89%（大=好），孤雁出群IC≈-8.20%（大=差）。
#    水中行舟=大值未来收益低，IC应为负。
#    故合成应为：gu_yan（大=差） - sui_bo（大=好） = 小值=好股票 → 合成值大=差 → IC为负 ✅
#
# 2. 【修复二】随波逐流Spearman矩阵：共同交易日全列非NaN筛选 → 改为逐对(pairwise)共同有效数据
#    原common_mask要求窗口内所有股票每一天都有值，导致A股5000只中任一只停牌即丢弃整天，
#    20日窗口共同交易日常锐减至3~5天，大量月末无因子值。现逐对只在两者都有值的日期上计算。
#
# 3. 【修复三】孤雁出群Pearson矩阵：列均值填充NaN + 矩阵运算 → 改为逐对(pairwise)共同有效数据
#    列均值填充使缺失时刻的成交额变为同一常数，人为制造虚假相关，彻底污染"成交额独立性"逻辑。
#    回退到逐对共同有效数据计算Pearson，与v3.7原本已有的 _pearson_corr_matrix_pairwise 一致。
#
# 4. 【修复四】删除截面均值填充：删除合成前的 groupby(trade_date).fillna(mean)
#    研报无此步骤。停牌/数据不足股票用截面均值填充会人为压缩头尾区分度，稀释IC。
#
...
def fetch_minute_data_with_market(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    try:
        ...
        # 剔除集合竞价数据（9:15-9:25），仅保留连续竞价时段（9:30-15:00）
...
@numba.njit(parallel=True)
def _process_single_window_pairwise(window_data, min_periods, min_stocks):
    """
    【v3.8修复】单个20日窗口内，逐对(pairwise)共同有效数据计算Spearman相关系数。

    修复原common_mask要求所有股票共同有值的致命缺陷：
    A股任一只股票停牌即丢弃整天，导致20日窗口有效天数锐减。
    现改为：对每对股票，只使用两者都有高低额差值的日期计算Spearman。
    """
    ...
    # 4. 【核心修复】逐对计算Spearman（只使用共同非NaN的日期）
    ...
    for j in numba.prange(n_valid):
        ...
        for i in range(n_valid):
            ...
            # Pearson相关（rank上的Pearson = Spearman）
            mx = 0.0
            my = 0.0
            for k in range(n_common):
                mx += x[k]
...
# ========== 【修复三】孤雁出群：逐对(pairwise)共同有效数据计算Pearson ==========

@numba.njit(parallel=True)
def _pearson_corr_matrix_pairwise_parallel(data: np.ndarray) -> np.ndarray:
    """
    【v3.8修复】逐对计算Pearson相关系数矩阵，使用共同有效数据。

    修复v3.7列均值填充NaN导致的虚假相关问题：
    列均值填充使缺失值变为同一常数，人为制造"所有缺失股票成交额相同"的虚假关联，
    彻底污染"成交额独立性"因子逻辑。

    对每对列只使用两者都非NaN的数据计算Pearson。已加parallel=True加速。
    """
...
def calculate_sui_bo_zhu_liu_vectorized(df_high_low_diff: pd.DataFrame) -> pd.DataFrame:
    """
    【修复二】计算随波逐流因子——逐对(pairwise)Spearman相关系数。
    """
    ...
    for i in range(FACTOR_WINDOW - 1, n_dates):
        ...
        # 【修复二】使用逐对pairwise版本
        valid_indices, avg_corr = _process_single_window_pairwise(
        ...
        if (i - FACTOR_WINDOW + 2) % 50 == 0 or i == n_dates - 1:
            print(f"  进度: {i - FACTOR_WINDOW + 2}/{n_dates - FACTOR_WINDOW + 1} "
                  f"({(i - FACTOR_WINDOW + 2)/(n_dates - FACTOR_WINDOW + 1)*100:.1f}%)")
...
def calculate_gu_yan_chu_qun_vectorized(df_minute: pd.DataFrame) -> pd.DataFrame:
    """
    【修复三】向量化计算孤雁出群因子——逐对(pairwise)共同有效数据Pearson相关。

    修正二保留：分钟收益率 groupby(['instrument','trade_date'])
    修正三升级：删除列均值填充，回退到逐对共同有效数据计算Pearson
    """
    if df_minute.empty:
        return pd.DataFrame()
    ...
    print(f"逐对(pairwise)共同有效数据Pearson相关（无NaN填充）...")
    ...
    for trade_date in all_dates:
        ...
        # 【v3.8修复三】回退到逐对pairwise计算，禁止NaN填充
        corr_matrix = _pearson_corr_matrix_pairwise_parallel(sub_matrix)
...
def calculate_daily_factors_vectorized(start_date: str, end_date: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("\n【修复二】计算随波逐流因子（逐对pairwise Spearman）...")
    df_sui_bo = calculate_sui_bo_zhu_liu_vectorized(df_high_low_diff)

    print("\n【修复三】计算孤雁出群因子（逐对pairwise Pearson，无NaN填充）...")
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（水中行舟）因子计算 - v3.8 BUG修复版 ===")
    ...
    print("   随波逐流大=好(+向)，孤雁出群大=差(-向)。水中行舟IC应为负，原公式把符号搞反了。")
    print("2. 【修复二】随波逐流：共同交易日全列非NaN → 逐对(pairwise)共同有效数据Spearman")
    print("   原common_mask任一股停牌即丢整天，20日窗口常剩3~5天，大量月末无因子值。已修复。")
    print("3. 【修复三】孤雁出群：列均值填充NaN + 矩阵运算 → 逐对(pairwise)共同有效数据Pearson")
    print("   列均值填充人为制造虚假相关，彻底污染成交额独立性。回退到pairwise计算。")
