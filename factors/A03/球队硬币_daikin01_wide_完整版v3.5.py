...
# 版本：v3.5 - 修复波动翻转子因子逻辑（逐日判断→逐日翻转当日值→再20日均值）
# 修复：三个维度的波动翻转子因子原逻辑错误（翻转20日滚动均值而非当日值）
#       修正为：逐日判断硬币/球队→逐日翻转当日值→再计算20日滚动均值
#       对齐换手翻转v3.4的逻辑，保持波动翻转与换手翻转一致
#
# 因子构建逻辑：
...
def calculate_intraday_factor(df_intraday: pd.DataFrame) -> pd.DataFrame:
    ...
    # 原逻辑：用当日判断直接翻转20日收益率均值，与研报定义不符
    # 新逻辑：每天独立判断硬币/球队，翻转当日收益率，再求20日均值
    ...
    # 原逻辑：用当日判断直接翻转20日收益率均值，过度依赖当日判断
    # 新逻辑：每天独立判断硬币/球队，翻转当日收益率，再求20日均值
