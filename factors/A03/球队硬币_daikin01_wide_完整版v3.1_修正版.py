...
#   2. 修正日内反转：基于分钟数据（日内收益率 = close/open-1）
...
def calculate_intraday_factor(df_intraday: pd.DataFrame) -> pd.DataFrame:
    ...
    df['intraday_vol_std'] = df.groupby('instrument')['intraday_volatility'].transform(
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（球队硬币）因子计算 - 完整版v3.0 ===")
