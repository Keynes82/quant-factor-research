...
# 版本：v3.7 - BUG修复版（修复turn_change列未定义即使用的问题）
# 修复：
#   1. 【v3.7修复】calculate_intraday_factor() 入口增加 turn_change 列存在性检查
#      支持函数独立调用，不再依赖主流程预处理生成 turn_change
# 优化：
#   1. 分钟数据SQL层聚合（array_agg）：IO量减少95%，每只股票每天1行而非240行
#   2. 向量化三维度合成：替代apply(axis=1)逐行循环，提速50-100倍
#   3. 日频查询合并：维度1和维度2&3共用一次日频查询
#   4. 日内维度全向量化：单只股票日频聚合后直接计算，省去进程池开销
#
# v3.5修复内容保留：
#   - 波动翻转子因子：逐日判断→逐日翻转当日值→再20日均值（三个维度全部修复）
#   - 换手翻转子因子：逐日判断→逐日翻转当日值→再20日均值（v3.4修复）
#   - 日内波动率使用20天日内收益率序列的std（v3.3修复）
#
# 因子构建逻辑：
...
#
# 更新日志：
#   v3.5 (2026-04-25): 【修正】波动翻转子因子：逐日判断→逐日翻转当日值→再20日均值（三个维度）
#   v3.4 (2026-04-22): 【修正】换手翻转子因子：逐日判断→逐日翻转当日值→再20日均值
...
def fetch_stock_minute_data_batch(instruments: List[str], start_date: str, end_date: str) -> pd.DataFrame:
    """批量读取分钟级数据（SQL层array_agg聚合版）
    
    v3.6优化：使用array_agg将分钟数据聚合为每日序列，IO量减少约95%
    返回每只股票每天的：开盘、收盘、open_series、close_series、volume_series
    """
    if not instruments:
        return pd.DataFrame()
...
def extract_intraday_from_aggregated(df_minute_agg: pd.DataFrame) -> pd.DataFrame:
    """
    v3.6优化：从SQL聚合的分钟数据中提取日内统计量
    
    输入：array_agg聚合后的DataFrame，每行=一只股票一天
          列：trade_date, instrument, open_series, close_series, volume_series, n_minutes
    
    输出：日频DataFrame，列：date, instrument, intraday_return, intraday_volatility
    """
    if df_minute_agg is None or df_minute_agg.empty:
        return pd.DataFrame()
    
    df = df_minute_agg.copy()
    results = []
    ...
    for _, row in df.iterrows():
        ...
        if isinstance(open_series, list):
            open_arr = np.array(open_series, dtype=np.float64)
        elif isinstance(open_series, np.ndarray):
            open_arr = open_series.astype(np.float64)
        else:
            continue
        ...
        # 日内收益率 = 收盘/开盘 - 1
        daily_open = open_arr[0]
        daily_close = close_arr[-1]
        ...
        # 分钟收益率波动率（当日分钟序列的std）
        minute_returns = np.diff(close_arr) / close_arr[:-1]
        intraday_volatility = float(np.nanstd(minute_returns))
...
def calculate_intraday_factor(df_intraday: pd.DataFrame) -> pd.DataFrame:
    ...
    # 【v3.7修复】确保 turn_change 列存在（支持函数独立调用，不依赖主流程预处理）
    if 'turn_change' not in df.columns:
        if 'turn' in df.columns:
            df['turn_change'] = df.groupby('instrument')['turn'].diff()
        else:
            df['turn_change'] = np.nan
    
    # 确保 turn 列存在（用于后续截面均值计算）
    ...
    # 波动翻转
    # 【v3.5修正】波动翻转：逐日判断→逐日翻转当日收益率→再20日均值
    df['flipped_intraday_return_vol'] = np.where(df['is_coin_vol'],
                                                  -df['intraday_return'],
                                                  df['intraday_return'])
    df['vol_flip_intraday'] = df.groupby('instrument')['flipped_intraday_return_vol'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
    
    # 换手翻转
    # 【v3.4修正】换手翻转：逐日判断→逐日翻转当日收益率→再20日均值
    df['flipped_intraday_return'] = np.where(df['is_coin_turn'],
                                              -df['intraday_return'],
                                              df['intraday_return'])
    df['turn_flip_intraday'] = df.groupby('instrument')['flipped_intraday_return'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
...
def calculate_daytime_factor(df_daily_all: pd.DataFrame) -> pd.DataFrame:
    ...
    # 波动翻转
    # 【v3.5修正】波动翻转：逐日判断→逐日翻转当日收益率→再20日均值
    df['flipped_daytime_return_vol'] = np.where(df['is_coin_vol'],
                                                 -df['daytime_return'],
                                                 df['daytime_return'])
    df['vol_flip_daytime'] = df.groupby('instrument')['flipped_daytime_return_vol'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
    
    # 换手翻转
    # 【v3.4修正】换手翻转：逐日判断→逐日翻转当日收益率→再20日均值
    df['flipped_daytime_return'] = np.where(df['is_coin_turn'],
                                             -df['daytime_return'],
                                             df['daytime_return'])
    df['turn_flip_daytime'] = df.groupby('instrument')['flipped_daytime_return'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
...
def calculate_overnight_factor(df_daily_all: pd.DataFrame) -> pd.DataFrame:
    ...
    # 波动翻转（隔夜距离 × -1 或保持不变）
    # 【v3.5修正】波动翻转：逐日判断→逐日翻转当日隔夜距离→再20日均值
    df['flipped_overnight_distance_vol'] = np.where(df['is_coin_overnight_vol'],
                                                     -df['overnight_distance'],
                                                     df['overnight_distance'])
    df['vol_flip_overnight'] = df.groupby('instrument')['flipped_overnight_distance_vol'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
    
    # 换手翻转
    # 【v3.4修正】换手翻转：逐日判断→逐日翻转当日隔夜距离→再20日均值
    df['flipped_overnight_distance'] = np.where(df['is_coin_turn_distance'],
                                                 -df['overnight_distance'],
                                                 df['overnight_distance'])
    df['turn_flip_overnight'] = df.groupby('instrument')['flipped_overnight_distance'].transform(
        lambda x: x.rolling(window=FACTOR_WINDOW, min_periods=min_periods).mean()
    )
...
def merge_three_dimensions(df_intraday: pd.DataFrame, 
    ...
    v3.6优化：向量化合成，替代apply(axis=1)逐行循环
    """
    print(f"\n【三维度合成】")
    ...
    # v3.6优化：向量化合成（替代apply逐行循环）
    cols = ['revised_intraday_reversal', 'revised_daytime_reversal', 'revised_overnight_reversal']
    existing_cols = [c for c in cols if c in merged.columns]
    
    if existing_cols:
        # 只对有值的维度求平均（sum / count）
        valid_counts = merged[existing_cols].notna().sum(axis=1)
        merged['team_coin_factor'] = merged[existing_cols].sum(axis=1) / valid_counts
        merged['team_coin_factor'] = merged['team_coin_factor'].replace([np.inf, -np.inf], np.nan)
        
        # 全部缺失的股票剔除
        all_na = merged[existing_cols].isna().all(axis=1)
        merged.loc[all_na, 'team_coin_factor'] = np.nan
    else:
        merged['team_coin_factor'] = np.nan
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（球队硬币）因子计算 - 完整版v3.6（性能优化版） ===")
    print(f"配置: SQL层array_agg聚合 + 向量化合成")
    print(f"计算范围: {start_date} ~ {end_date}")
    print(f"因子窗口: {FACTOR_WINDOW}日")
    print(f"\n【更新说明】v3.6: 性能优化（SQL层聚合 + 向量化合成 + 日频查询合并）")
    print(f"\n【三维度合成】")
    print(f"  1. 修正日间反转：日频数据（日间收益率）")
    print(f"  2. 修正日内反转：分钟数据聚合（日内收益率，完整时段9:30-15:00）")
    ...
    # v3.6优化：预先统一查询日频数据（扩展日期，供三个维度共用）
    print("\n【数据准备】统一查询日频数据（供三个维度共用）...")
    ...
    if all_daily_data:
        ...
        print(f"日频数据总计: {len(df_daily_all)} 条，股票数: {df_daily_all['instrument'].nunique()}只")
    
    # ========== 维度1：修正日内反转（聚合分钟数据）==========
    print("\n" + "="*60)
    print("【维度1】修正日内反转因子（聚合分钟数据 → 向量化提取）")
    print("="*60)
    ...
    for batch_idx, batch in enumerate(chunk_list(instruments_all, CHUNK_SIZE), start=1):
        ...
        # 批量获取聚合分钟数据（v3.6优化：array_agg，每只股票每天1行）
        df_minute_agg = fetch_stock_minute_data_batch(batch, effective_start, end_date)
        
        if df_minute_agg is None or df_minute_agg.empty:
            continue
        
        # v3.6优化：向量化提取日内统计量（替代逐只股票循环）
        df_intraday_raw = extract_intraday_from_aggregated(df_minute_agg)
        
        if df_intraday_raw is None or df_intraday_raw.empty:
            continue
        
        # 合并换手率（从预查的日频数据中筛选当前批次）
        df_daily_batch = df_daily_all[df_daily_all['instrument'].isin(batch)].copy()
        if not df_daily_batch.empty:
            df_daily_batch['date'] = pd.to_datetime(df_daily_batch['date']).dt.normalize()
            df_intraday_raw['date'] = pd.to_datetime(df_intraday_raw['date']).dt.normalize()
            
            # merge换手率
            df_intraday_raw = df_intraday_raw.merge(
                df_daily_batch[['date', 'instrument', 'turn']], 
                on=['date', 'instrument'], how='left'
            )
            df_intraday_raw['turn_change'] = df_intraday_raw.groupby('instrument')['turn'].diff()
        else:
            df_intraday_raw['turn'] = np.nan
            df_intraday_raw['turn_change'] = np.nan
        
        all_intraday_results.append(df_intraday_raw)
        batch_time = (datetime.now() - batch_start).total_seconds()
        print(f"第 {batch_idx} 批完成：{len(df_intraday_raw)} 行，耗时 {batch_time:.2f}秒")
    ...
    # ========== 维度2&3：日间+隔夜（共用预查日频数据）==========
    print("\n" + "="*60)
    print("【维度2&3】修正日间反转 + 修正隔夜反转（共用日频数据）")
    print("="*60)
    
    # v3.6优化：直接使用预查的日频数据，无需重复查询
    print(f"直接使用预查日频数据: {len(df_daily_all)} 条")
    ...
    print(f"\n【版本】v3.6 (2026-04-26) - 性能优化版（SQL聚合 + 向量化合成 + 日频查询合并）")
