...
#      - 加权决策分 = 惊恐度 × 波动率 × 收益率
#
#   4. 草木皆兵因子 = (20日加权决策分均值 + 20日加权决策分标准差) / 2
#
# 优化点：
#   - 多进程并行计算（8核）
#   - 批量SQL查询（减少IO）
...
def process_single_stock_panic(inst: str, df_min: pd.DataFrame, market_returns: Dict) -> Optional[pd.DataFrame]:
    """
    单只股票草木皆兵相关指标计算（优化版）- 处理多日期数据
    """
    try:
        ...
        # 合并市场收益率
        df_daily['market_return'] = df_daily['date'].map(market_returns)
        
        # 如果市场数据缺失，使用当日所有股票平均作为代理
        if df_daily['market_return'].isna().all():
            df_daily['market_return'] = 0
        
        # 计算惊恐度 = |个股收益率 - 市场收益率| / (|个股收益率| + |市场收益率| + 0.1)
        df_daily['deviation'] = (df_daily['daily_return'] - df_daily['market_return']).abs()
        df_daily['benchmark'] = df_daily['daily_return'].abs() + df_daily['market_return'].abs() + 0.1
        df_daily['panic_degree'] = df_daily['deviation'] / df_daily['benchmark']
        
        # 计算加权决策分 = 惊恐度 × 波动率 × 收益率
        df_daily['weighted_score'] = df_daily['panic_degree'] * df_daily['daily_volatility'] * df_daily['daily_return']
        
        return df_daily[['date', 'instrument', 'daily_volatility', 'daily_return', 
                        'panic_degree', 'weighted_score']].dropna()
...
def parallel_process_batch(stock_data_dict: Dict[str, pd.DataFrame], 
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        future_to_inst = {
            executor.submit(process_single_stock_panic, inst, df, market_returns): inst 
...
def calculate_daily_factor_optimized(start_date: str, end_date: str, 
    ...
    for batch_idx, batch in enumerate(chunk_list(instruments_all, batch_size), start=1):
        ...
        if use_parallel and len(stock_groups) > 1:
            batch_results = parallel_process_batch(stock_groups, market_returns, MAX_WORKERS)
        else:
            batch_results = []
            for inst, df in stock_groups.items():
                result = process_single_stock_panic(inst, df, market_returns)
                if result is not None and not result.empty:
...
def calculate_panic_factor_optimized(df_daily: pd.DataFrame) -> pd.DataFrame:
    ...
    合成公式：
    1) 加权决策分 = 惊恐度 × 波动率 × 收益率
    2) 草木皆兵因子 = (20日加权决策分均值 + 20日加权决策分标准差) / 2
    """
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（草木皆兵）因子计算 - 高性能优化版 ===")
    ...
    print("2) 基于1分钟数据计算日波动率")
    print("3) 加权决策分 = 惊恐度 × 波动率 × 收益率")
    print("4) 草木皆兵因子 = (20日加权决策分均值 + 20日加权决策分标准差) / 2")
