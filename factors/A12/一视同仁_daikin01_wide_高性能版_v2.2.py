# A12 一视同仁因子 - 高性能优化版 v2.2
# 来源：方正证券-多因子选股系列研究之十八《一视同仁》（2024-05-23）
# 档案编号：FQ-20240320-001
# 版本：v2.2 - 性能优化版（去除进程池 + Numba窗口计算）
#
# 【v2.2 性能优化】
# 1. 【优化一】去除ProcessPoolExecutor：
#    单股计算中等复杂度，但进程池pickle+序列化开销占主导。
#    改为单进程直接循环处理，消除pickle开销。
#    预估提速：2-5x
# 2. 【优化二】Numba化5分钟窗口波动率计算：
#    原v2.1逐窗口Python循环调用pandas std（10+窗口/日），
#    改为Numba编译函数一次性计算所有窗口的样本标准差。
#    预估提速：3-5x
#
# 【v2.1 修复保留】
#   修复1（严重BUG）：boxcox过滤0值后，volume_changes索引直接映射到day_df导致时刻错位
#     修复：建立正量序列索引→原始索引的映射表，正确构建耀眼/黯淡5分钟
#   修复2（中等问题）：波动率计算将多个5分钟窗口合并算一个std，研报要求分别计算再求均值
#     修复：每个激增/骤降时刻的5分钟窗口分别计算波动率，再对所有窗口波动率求均值
#
# 因子构建逻辑（严格按研报原文）：
#   1. 对成交量做boxcox变换使其正态化（仅对非零成交量）
#   2. 计算每分钟成交量变化量（变换后相邻非零成交量的差值）
#   3. 定义"正态激增时刻"（变化量>mean+std）和"正态骤降时刻"（变化量<mean-std）
#   4. 将激增/骤降时刻及其后4分钟记为"正态耀眼/黯淡5分钟"
#   5. 波动公平度 = |各耀眼5分钟波动率均值 - 各黯淡5分钟波动率均值|
#   6. 收益公平度 = |激增时刻收益率均值 - 骤降时刻收益率均值|
#   7. 波动公平收益率 = 日内收益率 × 波动公平度
#   8. 收益公平收益率 = 日内收益率 × 收益公平度
#   9. 波动公平因子 = 20日波动公平收益率均值
#   10. 收益公平因子 = 20日收益公平收益率均值
#   11. 一视同仁因子 = 波动公平因子 + 收益公平因子（等权合成）
...
@numba.njit
def _build_windows_numba(trigger_indices, n_minutes, brilliant_minutes):
    """
    Numba构建5分钟窗口，返回[start, end]数组。

    trigger_indices: int32数组，触发时刻的原始索引
    n_minutes: 当日总分钟数
    brilliant_minutes: 窗口长度（默认5）

    返回: (n_windows, 2) 的int32数组，每行=[start, end)
    """
    n = len(trigger_indices)
    # 先计算有效窗口数量
    count = 0
    for i in range(n):
        idx = trigger_indices[i]
        end = idx + brilliant_minutes
        if end > n_minutes:
            end = n_minutes
        if end - idx >= 2:  # 至少2个数据点才能算std
            count += 1

    if count == 0:
        return np.empty((0, 2), dtype=np.int32)

    windows = np.empty((count, 2), dtype=np.int32)
    j = 0
    for i in range(n):
        idx = trigger_indices[i]
        end = idx + brilliant_minutes
...
@numba.njit
def _calc_window_std_numba(returns, windows):
    """
    Numba计算多个窗口的样本标准差。

    returns: float64数组，当日每分钟收益率（含nan）
    windows: (n_windows, 2) int32数组，每行=[start, end)

    返回: float64数组，每个窗口的样本标准差（nan表示无效窗口）
    """
    n = len(windows)
    ...
    for i in range(n):
        ...
        for j in range(start, end):
            if not np.isnan(returns[j]):
                d = returns[j] - mean_val
                var_sum += d * d

        stds[i] = np.sqrt(var_sum / (c - 1))
...
def process_single_stock_optimized(inst: str, df_min: pd.DataFrame,
    ...
    """
    单只股票处理（v2.1修复版）- 一视同仁因子核心逻辑

    【v2.1关键修复】
    1. 建立正量序列索引→原始索引映射，解决激增/骤降时刻错位问题
    2. 波动率计算改为"分别计算各5分钟窗口波动率，再求均值"
    """
    try:
        ...
        for trade_date, day_df in df_min.groupby('trade_date'):
            ...
            n_minutes = len(day_df)

            # === 步骤1：构建正量序列并做boxcox变换 ===
            # 研报原文："首先剔除成交量为0的时刻，将余下的时间重新排成一个新的序列"
            positive_mask = day_df['volume'].values > 0
            positive_indices = np.where(positive_mask)[0]  # 原始索引

            if len(positive_indices) < 10:
                continue

            positive_volumes = day_df['volume'].values[positive_indices]
            ...
            # === 步骤2：计算成交量变化量 ===
            # 研报原文："计算变换之后每一分钟相对其前一个成交量不为0的时间点的交易量变化量"
            volume_changes = np.diff(transformed)
            ...
            # === 步骤4：识别正态激增时刻和正态骤降时刻 ===
            # 研报原文："变化量大于mean+std的部分为正态激增时刻，小于mean-std的部分为正态骤降时刻"
            surge_threshold = mean_change + std_change
            drop_threshold = mean_change - std_change

            # volume_changes[i] = transformed[i+1] - transformed[i]
            # 对应 positive_indices[i+1] 和 positive_indices[i]
            # 激增/骤降时刻是 positive_indices[i+1]（变化量发生的位置）
            surge_indices_in_changes = np.where(volume_changes > surge_threshold)[0]
            drop_indices_in_changes = np.where(volume_changes < drop_threshold)[0]

            if len(surge_indices_in_changes) == 0 or len(drop_indices_in_changes) == 0:
                continue

            # 【v2.1修复】映射回原始索引
            surge_original_indices = positive_indices[surge_indices_in_changes + 1]
            drop_original_indices = positive_indices[drop_indices_in_changes + 1]

            # === 计算每分钟收益率 ===
            day_df['return'] = day_df['close'].pct_change()

            # === 步骤5：识别"正态耀眼五分钟"和"正态黯淡五分钟" ===
            # 研报原文："将正态激增时刻的1分钟及其随后的4分钟记为正态耀眼五分钟"
            #           "将正态骤降时刻的1分钟及其随后的4分钟记为正态黯淡五分钟"
            # 【v2.2优化】Numba构建窗口

            brilliant_windows = _build_windows_numba(
                surge_original_indices.astype(np.int32),
                n_minutes,
                brilliant_minutes
            )
            dim_windows = _build_windows_numba(
                drop_original_indices.astype(np.int32),
                n_minutes,
                brilliant_minutes
            )

            if len(brilliant_windows) == 0 or len(dim_windows) == 0:
                continue

            # === 步骤6：计算波动公平度 ===
            # 研报原文："对这一天所有正态耀眼波动率与正态黯淡波动率分别求均值"
            # 【v2.1修复】每个5分钟窗口分别计算波动率，再对所有窗口波动率求均值
            # 【v2.2优化】Numba计算窗口波动率

            returns_arr = day_df['return'].values.astype(np.float64)

            brilliant_vols_raw = _calc_window_std_numba(returns_arr, brilliant_windows)
            brilliant_vols = brilliant_vols_raw[~np.isnan(brilliant_vols_raw)]

            dim_vols_raw = _calc_window_std_numba(returns_arr, dim_windows)
            dim_vols = dim_vols_raw[~np.isnan(dim_vols_raw)]

            if len(brilliant_vols) == 0 or len(dim_vols) == 0:
                continue

            brilliant_vol = np.mean(brilliant_vols)
            dim_vol = np.mean(dim_vols)

            if pd.isna(brilliant_vol) or pd.isna(dim_vol):
                continue

            # 波动公平度 = |耀眼波动率均值 - 黯淡波动率均值|
            volatility_fairness = abs(brilliant_vol - dim_vol)

            # === 步骤7：计算收益公平度 ===
            # 研报原文："计算当天所有正态激增时刻与正态骤降时刻的收益率...分别求均值"
            # 【v2.1修复】使用正确映射的原始索引
            surge_returns = day_df.iloc[surge_original_indices]['return']
            drop_returns = day_df.iloc[drop_original_indices]['return']

            surge_returns = surge_returns.dropna()
            drop_returns = drop_returns.dropna()
...
def calculate_daily_factor_optimized(start_date: str, end_date: str,
    ...
    print(f"获取到 {len(instruments_all)} 只股票，批次大小={batch_size}，单进程直接处理")
    ...
    for batch_idx, batch in enumerate(chunk_list(instruments_all, batch_size), start=1):
        ...
        # 【v2.2优化】单进程直接循环处理（去除ProcessPoolExecutor）
        batch_results = []
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（一视同仁）因子计算 - v2.2性能优化版 ===")
    ...
    print(f"【v2.2优化】去除进程池 + Numba窗口计算")
    print(f"【v2.1修复】索引映射 + 波动率分别计算")
    ...
    if df_daily is None or df_daily.empty:
        ...
        if df_factor is None or df_factor.empty:
            ...
            if not df_factor.empty:
                ...
                print(f"\nA12一视同仁因子 v2.2 - 性能优化版（去除进程池 + Numba窗口计算）")
                print(f"【v2.1修复】索引映射 + 波动率分别计算")
                print(f"预期表现: Rank IC ~ -7.39%")
