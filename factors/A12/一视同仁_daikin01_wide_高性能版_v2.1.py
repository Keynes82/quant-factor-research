...
def process_single_stock_optimized(inst: str, df_min: pd.DataFrame, 
    ...
    try:
        ...
        for trade_date, day_df in df_min.groupby('trade_date'):
            ...
            def build_five_minute_periods(trigger_indices, max_len):
                """构建5分钟窗口（触发时刻+后4分钟），返回各窗口的原始索引列表"""
                periods = []
                for idx in trigger_indices:
                    window = [idx + i for i in range(brilliant_minutes) if idx + i < max_len]
                    if len(window) >= 2:  # 至少2个数据点才能算波动率
                        periods.append(window)
                return periods
            
            brilliant_periods = build_five_minute_periods(surge_original_indices, n_minutes)
            dim_periods = build_five_minute_periods(drop_original_indices, n_minutes)
            ...
            brilliant_vols = []
            for window in brilliant_periods:
                window_returns = day_df.iloc[window]['return']
                if len(window_returns.dropna()) >= 2:
                    brilliant_vols.append(window_returns.std())
            
            dim_vols = []
            for window in dim_periods:
                window_returns = day_df.iloc[window]['return']
                if len(window_returns.dropna()) >= 2:
                    dim_vols.append(window_returns.std())
...
if __name__ == "__main__":
    ...
    print(f"=== {FACTOR_NAME}（一视同仁）因子计算 - v2.1修复版 ===")
